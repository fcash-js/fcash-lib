...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new fcash_base.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new fcash_base.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = fcash_base.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new fcash_base.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function fcash_baseTest() {
      block1 = fcash_base.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('fcash_base', fcash_baseTest, {maxTime: maxTime});
